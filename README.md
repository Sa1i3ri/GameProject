...
1. 拖入预制体dialog_canvas
2. 在Assets/Texts文件夹中新建txt文件，一句（就是一次要在对话框中显示的文本量）一行，每一关的一连串句子或对话单独保存一个文件（因为一次就要把整个文件放完才能退出）
3. 在预制体下面的panel中，有个脚本，在脚本的文本文件中拖动加入这关要用的txt文件即可，剩下的参数都别动
